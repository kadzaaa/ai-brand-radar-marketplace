---
name: audit-brand-answers
description: Audit how Claude answers 50 high-intent questions about a real brand, which sources it cites, where competitors replace the brand, whether product information is correct and complete, and what exact website changes could improve future Claude answers. Use for Claude brand visibility audits, citation audits, answer-engine optimization, competitor answer comparisons, website content-gap analysis, post-update rechecks, and comparisons with a previous AI Brand Radar scan. Do not use this skill to claim access to global Claude analytics or internal Anthropic ranking data.
---

# Audit Claude Brand Answers

Run an evidence-backed, controlled audit centered on one outcome: help users receive accurate, useful information about the brand when they ask Claude commercially important questions. Treat every result as an observation from the current research run, never as global Claude telemetry.
...
Use the GEO language only for the exact 50 Claude prompts and their verbatim recheck versions. Keep URLs, registered brand and product names, and machine-readable schema keys unchanged. Summarize or translate source evidence into Russian instead of switching the report language.
...
```text
Проведу аудит 50 популярных и коммерчески значимых запросов пользователей в Claude, оценю представленность и цитируемость бренда, а затем дам конкретные рекомендации по контенту и страницам сайта, которые могут повысить вероятность появления бренда в ответах Claude.
...
Call the panel `high-intent` unless reliable demand data supports `popular`. Never invent Claude prompt volume.
...
Call prompts `popular` only when demand evidence supports that label. Otherwise call them `frequent themes according to public demand proxies` or `high-intent prompts`; state the supporting signal and never invent Claude prompt volume.
...
Include methodology and limitations. Use the Russian label «Присутствие бренда в наблюдаемых ответах Claude», not global `Claude market share` or `share of voice` without qualification.
...
- the report discloses that it is a controlled current-run observation, not internal Claude analytics.
